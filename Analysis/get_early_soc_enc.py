...
def getcoauthorearly(NowDic, Rawtitles, topkcoauthor):
    NewDic = {}
    for key in NowDic:
            ...
            for author in authors:
                ...
                if each_name[-1] == last_name:
                    if each_name[0][0] == first_letter:
                        mini = min(mini, int(rawtitle[-1]))                 
        NewDic[key] = mini
    # print(len(NewDic))
    # print(NowDic)
    Sorted_NewDic = sorted(NewDic.items(), key=lambda x: x[1])
...
def dealsocenrichment():
    ...
    for suffix in list_suffix:
        ...
        for msg in tqdm(msgs[:topk]):
            ...
            Coauthor_Rawtitles_list, Coauthors_count = getcoauthorearly(NowDic, Rawtitles, topkcoauthor)
    ...
    sio.savemat('./Ana-soc-enr-early-BERT.mat', Sdic)
